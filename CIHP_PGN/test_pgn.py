...
def get_opt():
    ...
    parser.add_argument('--data_dir', type=str, default='./datasets/CIHP')
    parser.add_argument('--list_path', type=str, default='./datasets/CIHP/list/val.txt')
    parser.add_argument('--data_id_list', type=str, default='./datasets/CIHP/list/val_id.txt')
    parser.add_argument('--checkpoint', type=str, default='./checkpoint/CIHP_pgn')
    parser.add_argument('--parsing_dir', type=str, default='./output/cihp_parsing_maps')
    parser.add_argument('--ref_image', type=str, default='./datasets/palette_ref.png')
...
def main():
    ...
    with open(opt.data_id_list, 'r') as f:
    ...
    with tf.name_scope("create_inputs"):
        reader = ImageReader(opt.data_dir, opt.list_path, opt.data_id_list, None, False, False, False, coord)
    ...
    # evaluate prosessing
    if not os.path.exists(opt.parsing_dir):
        os.makedirs(opt.parsing_dir)
    '''
    ...
    for step in range(NUM_STEPS):
        ...
        label = parsing_[0,:,:,0].astype(np.uint8)

        palette_ref = Image.open(opt.ref_image)
        img = Image.fromarray(label, 'P')
        img.palette = palette_ref.palette

        img.save('{}/{}.png'.format(opt.parsing_dir, img_id))
        print('save parse image: {}.png'.format(img_id))

        #msk = decode_labels(parsing_, num_classes=N_CLASSES)
        #parsing_im = Image.fromarray(msk[0])
        #parsing_im.save('{}/{}_vis.png'.format(opt.parsing_dir, img_id))
        #cv2.imwrite('{}/{}.png'.format(opt.parsing_dir, img_id), parsing_[0,:,:,0])
